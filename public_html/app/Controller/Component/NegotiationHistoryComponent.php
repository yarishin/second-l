...
App::uses('CakeEmail', 'Network/Email');
 ...
 //拡張クラスを作成
class NegotiationHistoryComponent extends Component {
	...
	/**
	 * 交渉履歴新規登録<br>
	 * 交渉履歴登録画面で登録ボタンを押下した際の処理<br>
	 * @param array $data
	 * @throws Exception
	 * 
	 */
	function insertNegotiationHistory($data) {
		...
		try {
			...
			$reg_data = array(
			   ...
			   ,COLUMN_0000010 => date(DATETIME_FORMAT)
		   ...
		   $this->Controller->TrnNegotiationHistory->create();
		   $this->Controller->TrnNegotiationHistory->save($reg_data, false);
			
		} catch (Exception $ex) {
			$err = "NegotiationHistory->insertNegotiationHistory で例外発生<br>";
	 ...
	 * @param type $user_id
	 */
	function getNegotiationHistory($user_id, $user_name) {
		try {
			...
			$conditions[COLUMN_0000070] = null;
			$status[MODEL_CONDITIONS]   = $conditions;
			
			$order = null;
			$order[COLUMN_1900030] = "desc";
			$status[MODEL_ORDER] = $order;
			$negotiation_data = $this->Controller->TrnNegotiationHistory->find(MODEL_ALL, $status);
			...
			foreach ($negotiation_data as $key => $values) {
				foreach ($values as $value) {
					...
					$data[OBJECT_ID_050300110.$count] = $value[COLUMN_0000050]." ： ".$value[COLUMN_0000060];
					//$data[OBJECT_ID_050300120.$count] = $value[COLUMN_0000060];
					$count++;
				}
			}
			...
			$err = "User->getNegotiationHistory で例外発生<br>";
			throw new Exception($err.$ex->getMessage()."<br>");
		}
	}
	...
	/**
	 * 交渉履歴更新<br>
	 * 交渉履歴画面で更新、または削除ボタンを押下した場合の処理。<br>
	 * $flag = true : 更新処理<br>
	 * $flag = false : 削除処理<br>
	 * @param array $data
	 * @param number $count
	 ...
	 * @throws Exception
	 */
	function updateNegotiationHistory($data, $count, $flag) {
		try {
			...
			if ($flag) {
				...
				$reg_data = array(
					COLUMN_1900000 => $data[OBJECT_ID_050300100.$count]
				   ,COLUMN_0000070 => date(DATETIME_FORMAT)
				   ,COLUMN_0000080 => $admin_info[LOGIN_ADMIN_ID]
				   ,COLUMN_0000090 => $admin_info[LOGIN_ADMIN_NAME]
				);
			}
			...
			$err = "User->updateNegotiationHistory で例外発生<br>";
			throw new Exception($err.$ex->getMessage()."<br>");
		}
	}
}
